...
### Фаза 1: Социальные функции - КРИТИЧЕСКИЙ ПРИОРИТЕТ

#### 1.1 Система мотоклубов
- [ ] `app/domain/entities/moto_club.py` - сущность мотоклуба
- [ ] `app/domain/entities/club_membership.py` - членство с ролями
- [ ] `app/domain/entities/club_invitation.py` - приглашения в клубы
- [ ] Роли: President, Vice-President, Secretary, Member + кастомные
- [ ] Система заявок на вступление и приглашений
- [ ] Права ролей (создание событий, модерация, управление)
- [ ] API для управления клубами и участниками

#### 1.2 Система дружбы
- [ ] `app/domain/entities/friendship.py` - связи между пользователями
- [ ] `app/domain/entities/friend_request.py` - заявки в друзья
- [ ] Статусы: pending, accepted, blocked
- [ ] Интеграция с системой приватности профилей
- [ ] Уведомления о заявках в друзья

#### 1.3 События и мероприятия - КЛЮЧЕВАЯ ФУНКЦИЯ
...
**Критический путь к MVP (6-10 недель):**
1. **Мотоклубы** (2 недели) - основа социального взаимодействия
2. **События** (2 недели) - ключевая функция для встреч
3. **Дружба** (1 неделя) - улучшение приватности
4. **POI система** (2 недели) - каталог важных мест
5. **Базовые карты** (1 неделя) - отображение событий и POI
6. **Android приложение** (2 недели) - основной клиент

**Текущий прогресс:** 40% готовности к MVP. Завершена вся базовая инфраструктура.