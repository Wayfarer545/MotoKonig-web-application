# app/infrastructure/specs/motokonig/motokonig_by_id.py

from typing import Any
from uuid import UUID

from app.domain.ports.specs.motokonig import MotoKonigSpecificationPort
from app.infrastructure.models.motokonig import MotoKonig as MotoKonigModel


class MotoKonigById(MotoKonigSpecificationPort):
    """Спецификация поиска по ID"""

    def __init__(self, motokonig_id: UUID):
        self.motokonig_id = motokonig_id

    def to_query(self, base_query: Any) -> Any:
        return base_query.where(MotoKonigModel.id == self.motokonig_id)
