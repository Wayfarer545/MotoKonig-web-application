...
from app.application.use_cases.listing_category.create_category import CreateListingCategoryUseCase
from app.application.use_cases.listing_category.get_category import GetListingCategoryUseCase
from app.application.use_cases.listing_category.list_categories import ListListingCategoriesUseCase
...
class UseCaseProvider(Provider):
    ...
    @provide(scope=Scope.REQUEST)
    def provide_publish_listing_uc(self, repo: IListingRepository) -> PublishListingUseCase:
        return PublishListingUseCase(repo)

    @provide(scope=Scope.REQUEST)
    def provide_view_listing_uc(self, repo: IListingRepository) -> ViewListingUseCase:
        return ViewListingUseCase(repo)

    @provide(scope=Scope.REQUEST)
    def provide_search_listings_uc(self, repo: IListingRepository) -> SearchListingsUseCase:
        return SearchListingsUseCase(repo)

    # Category Use Cases
    @provide(scope=Scope.REQUEST)
    def provide_create_category_uc(self, repo: IListingCategoryRepository) -> CreateListingCategoryUseCase:
        return CreateListingCategoryUseCase(repo)

    @provide(scope=Scope.REQUEST)
    def provide_get_category_uc(self, repo: IListingCategoryRepository) -> GetListingCategoryUseCase:
        return GetListingCategoryUseCase(repo)

    @provide(scope=Scope.REQUEST)
    def provide_list_categories_uc(self, repo: IListingCategoryRepository) -> ListListingCategoriesUseCase:
        return ListListingCategoriesUseCase(repo)